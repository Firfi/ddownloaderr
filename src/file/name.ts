...
export const siteRootPrefix = (url: URL) => `${slugifyUrl(url)}-${hashUrl(url)}`;
export const staticsDirName = (url: URL) => siteRootPrefix(url) + "_files";
export const staticsFilePath = (root: URL, url: URL): string => `${staticsDirName(root)}/${hashUrl(url)}${tryExtension(url)}`
