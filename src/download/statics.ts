...
export const fetchStatics = async (
  ...
  const srcToMapped = new Map(
    page
      .peekSrcs()
      .map(({ src }) => [src, tryStaticSrc(root, src)] as const)
      .filter(([, url]) => !!url)
  );
  return new Map(
    (
      await Promise.all(
        [...srcToMapped.entries()].map(async ([src, mapped]) => {
          const buffer = await downloadStatics(mapped);
          if (!buffer) {
            if (args.debug)
              console.debug(`Failed to download statics ${mapped.href}`);
            return null;
          }
